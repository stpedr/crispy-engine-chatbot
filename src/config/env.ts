...
const envSchema = z.object({
  ...
  OLLAMA_ENABLED: booleanFromEnv.optional(),
  OLLAMA_BASE_URL: z.string().url().default("http://127.0.0.1:11434"),
  OLLAMA_MODEL: z.string().min(1).default("llama3:latest"),
  OLLAMA_TEMPERATURE: z.coerce.number().min(0).max(1).default(0.2),
  OLLAMA_TIMEOUT_MS: z.coerce.number().int().positive().default(120000),
  OLLAMA_MAX_HISTORY: z.coerce.number().int().positive().max(20).default(8),
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    ollamaEnabled: parsed.OLLAMA_ENABLED ?? parsed.NODE_ENV !== "test",
    ollamaBaseUrl: parsed.OLLAMA_BASE_URL.replace(/\/$/, ""),
    ollamaModel: parsed.OLLAMA_MODEL,
    ollamaTemperature: parsed.OLLAMA_TEMPERATURE,
    ollamaTimeoutMs: parsed.OLLAMA_TIMEOUT_MS,
    ollamaMaxHistory: parsed.OLLAMA_MAX_HISTORY,
